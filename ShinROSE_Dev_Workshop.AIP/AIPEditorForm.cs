...
namespace ShinROSE_Dev_Workshop.AIP
{
	public class AIPEditorForm : Form
	{
		...
		private int idle_check;

		private int damage_check;
		...
		private void openToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (this.openFileDialog.ShowDialog() == DialogResult.OK)
			{
				...
				this.load_aip_info(this.openFileDialog.FileName);
				this.StatusLabel.Text = "Statut : " + this.openFileDialog.FileName + "succefully load";
				this.Text = "AIP : \"" + this.openFileDialog.FileName + "\"";
			}
		...
		public void load_aip_info(string path)
		{
			this.contextMenuStripAction.Enabled = true;
			this.contextMenuStripCondition.Enabled = true;
			this.contextMenuStripData.Enabled = true;
			...
			this.idle_check = binaryReader.ReadInt32();
			this.labelFileVersion.Text = "File version :" + this.idle_check.ToString();
			this.damage_check = binaryReader.ReadInt32();
			int count = binaryReader.ReadInt32();
			this.title = Encoding.UTF7.GetString(binaryReader.ReadBytes(count));
			this.textBoxQSDName.Text = this.title;
			this.block = new AIPEditorForm.Blck[this.block_count];
			for (int num = 0; num != this.block_count; num++)
			{
				this.block[num].block_name = Encoding.UTF7.GetString(binaryReader.ReadBytes(32));
				...
				{
					this.block[num].record[num2].record_name = Encoding.UTF7.GetString(binaryReader.ReadBytes(32));
				...
				if (parent.Index == 0)
				{
					...
					if (condition.part_count("0x" + this.block[parent3.Index].record[parent2.Index].condition[this.treeView.SelectedNode.Index].command.ToString("X"), ref array, ref array2, this.executableDirectoryName))
					{
						...
						for (int num = 0; num != array.Length; num++)
						{
							...
							else if (array2[num] == "Empty32")
							{
								this.dataGridView.Rows[0].Cells[num].Value = "";
								num2 += 4;
							}
							else if (array2[num] == "Empty24")
							{
								this.dataGridView.Rows[0].Cells[num].Value = "";
								num2 += 3;
							}
							else if (array2[num] == "Empty16")
							{
								this.dataGridView.Rows[0].Cells[num].Value = "";
								num2 += 2;
							}
							else if (array2[num] == "Empty")
							{
								this.dataGridView.Rows[0].Cells[num].Value = "";
								num2++;
							}
		...
		private void renameToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			description.labelName.Text = "Rename(32 chars max) :";
		...
		private void saveAsToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			this.Text = "AIP : \"" + this.saveFileDialog.FileName + "\"";
		}
		...
		private void save_qsd(string path)
		{
			...
			binaryWriter.Write(this.idle_check);
			binaryWriter.Write(this.damage_check);
			binaryWriter.Write(Convert.ToInt32(this.title.Length));
			byte[] bytes = Encoding.Default.GetBytes(this.title);
			...
			for (int num = 0; num != this.block.Length; num++)
			{
				if (!this.block[num].deleted)
				{
					...
					array = Encoding.Default.GetBytes(this.block[num].block_name);
					...
					for (int num2 = 0; num2 != this.block[num].record.Length; num2++)
					{
						if (!this.block[num].record[num2].deleted)
						{
							...
							array2 = Encoding.Default.GetBytes(this.block[num].record[num2].record_name);
				...
				if (parent.Index == 0)
				{
					...
					else
					{
						...
						for (int num = 0; num != array3.Length; num++)
						{
							...
							else if (array3[num] == "Empty32")
							{
								this.block[parent3.Index].record[parent2.Index].condition[selectedNode.Index].data[num2] = 0;
								num2 += 4;
							}
							else if (array3[num] == "Empty24")
							{
								this.block[parent3.Index].record[parent2.Index].condition[selectedNode.Index].data[num2] = 0;
								num2 += 3;
							}
							else if (array3[num] == "Empty16")
							{
								this.block[parent3.Index].record[parent2.Index].condition[selectedNode.Index].data[num2] = 0;
								num2 += 2;
							}
							else if (array3[num] == "Empty")
							{
								this.block[parent3.Index].record[parent2.Index].condition[selectedNode.Index].data[num2] = 0;
								num2++;
							}
				...
				else if (parent.Index == 1)
				{
					...
					else
					{
						...
						for (int num = 0; num != array3.Length; num++)
						{
							...
							{
								this.block[parent3.Index].record[parent2.Index].action[selectedNode.Index].data[num2] = 0;
								num2 += 4;
							}
							else if (array3[num] == "Empty24")
							{
		...
		{
			this.title = this.textBoxQSDName.Text;
		}
		...
		private void InitializeComponent()
		{
			...
			this.labelQSDName.Size = new Size(61, 13);
			this.labelQSDName.TabIndex = 2;
			this.labelQSDName.Text = "AIP Name :";
			...
			this.openFileDialog.Filter = "Fichier AIP|*.aip";
			...
			this.saveFileDialog.Filter = "Fichier AIP|*.aip";
